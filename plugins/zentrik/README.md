# Zentrik plugin for Claude Code

Connects Claude to the product context your team keeps in [Zentrik](https://zentrik.ai): customer signals, insights, ideas, opportunities, initiatives, Studies, accounts, OKRs, product context, personas, and feature maps.

The plugin bundles the production Zentrik remote MCP connector with four workflow skills.
...
Run `/reload-plugins` to activate it in a running session.
...
You get back the initiative behind the work, what customers actually said, and the questions still open, each with the Zentrik record ID so you can check it.
...
Claude also invokes them automatically when the task matches. Every skill reads before it writes, asks for approval before mutating a workspace, and reports what a tool actually returned rather than claiming success.

## Authentication

The bundled connector points at `https://zentrik.ai/mcp`. Claude opens Zentrik OAuth when authentication is required.

During consent you choose one Zentrik workspace. Access stays bounded by that workspace, your Zentrik role, and the granted OAuth scopes. Read-only use works for Viewer roles; creating, updating, or importing records requires a write-capable role such as Editor, Admin, or Owner.
...
- Setup guide: https://zentrik.ai/docs/integrations/mcp-claude
